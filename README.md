...
The container selection method runs when the DOM is changed. Using a [muation observer](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver) the extension observes any created nodes. The extension keeps a master list of all of the nodes on the page along with metrics about the node. These metrics include the number of total added nodes, total removed nodes, number of times that the node has been changed and the current number of child nodes. When the master node list is updated all of the nodes are run through a function that ranks them based on an evaluation function. The evaluation function takes all of the nodes metrics and assigns an overall value for the node. The highest ranked nodes are put into a list of nodes that are most likely to contain the content for an area. 

Each content area has its own evalutaion function. The main content's evaluation function looks at the number of child nodes that a node has added over the lifetime of the page. This metric works well for identifying the main content for infinitely scrolling web sites such as reddit. There is room in this module for adding new content areas; however, none have been implemented for the current version of the project. 

There is also an experimental evaluation function. This method had the same results as the naive metrics using only the number of child nodes for infinitely scrolling websites. This method was more expensive than the current method by an order of magnitude so it was not used in the current version of the project. However, it has promise for future iterations and will need improvements to target other sites. This method gives weight to space on the page, where the center of the page has the highest weight since displaying ads at the center of the page will hinder user experience and seems unlikely. The next consideration is that larger containers are more likely to contain ads, but containers that are too large are likely to contain more than just ads. By using a quadratic equation to give weights to element size, we give higher weights to elements that take up half of the screen and lower weights to elements that take up less or more than half. By using these two metrics in combination, large containers near the center of the page have high weights, but containers that are too large are not considered. 
